...
public class MethodInstrumenter extends MethodVisitor
	...
	public void visitInsnWithProbe(final int opcode, final int probeId) {
		probeInserter.insertProbe(probeId);
		probeInserter.visitInsn(opcode);
	}
	...
	public void visitJumpInsnWithProbe(final int opcode, final Label label,
		...
		if (opcode == Opcodes.GOTO) {
			probeInserter.insertProbe(probeId);
			probeInserter.visitJumpInsn(Opcodes.GOTO, label);
		} else {
			final Label intermediate = new Label();
			probeInserter.visitJumpInsn(getInverted(opcode), intermediate);
			probeInserter.insertProbe(probeId);
			probeInserter.visitJumpInsn(Opcodes.GOTO, label);
			
			
			/*
			I_FrameInfo fi = probeInserter.getFrameInfo();
			mv.visitFrame(fi.getType(), 
					fi.getLocalSize(), 
					fi.getLocal(), 
					fi.getStackSize(), 
					fi.getStack());
	...
	public void visitTableSwitchInsnWithProbes(final int min, final int max,
		...
		probeInserter.visitTableSwitchInsn(min, max, newDflt, newLabels);
	...
	public void visitLookupSwitchInsnWithProbes(final Label dflt,
		...
		final Label newDflt = createIntermediate(dflt);
		final Label[] newLabels = createIntermediates(labels);
		probeInserter.visitLookupSwitchInsn(newDflt, keys, newLabels);
	...
	private void insertIntermediateProbe(final Label label, final IFrame frame) {
		...
		if (probeId != LabelInfo.NO_PROBE && !LabelInfo.isDone(label)) {
			probeInserter.visitLabel(LabelInfo.getIntermediateLabel(label));
			frame.accept(probeInserter);
			probeInserter.insertProbe(probeId);
			probeInserter.visitJumpInsn(Opcodes.GOTO, label);
			LabelInfo.setDone(label);
